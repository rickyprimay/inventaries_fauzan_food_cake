<div id="outlet-delete-modal-{{ $outlet->id }}" tabindex="-1"
                class="hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full h-full">
                <div class="relative p-4 w-full max-w-md max-h-full">
                    <div class="bg-white rounded-lg shadow-sm">
                        <button type="button" class="absolute top-3 end-2.5 text-gray-400 hover:text-gray-900" data-modal-hide="outlet-delete-modal-{{ $outlet->id }}">
                            &times;
                        </button>
                        <div class="p-4 text-center">
                            ...
                            <h3 class="mb-5 text-lg font-normal text-gray-500">Apakah anda yakin ingin menghapus outlet ini?</h3>
                            <form method="POST" action="{{ route('outlet.destroy', $outlet->id) }}">
                              ...
                              @method('DELETE')
                                ...
                                <button type="button" data-modal-hide="outlet-delete-modal-{{ $outlet->id }}" class="px-5 py-2.5 ms-3 border rounded-lg">Batalkan</button>
                            </form>
                        </div>
                    </div>
                </div>
