<div id="category-delete-modal-{{ $category->id }}" tabindex="-1"
  class="hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full h-full">
  <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="bg-white rounded-lg shadow-sm">
          <button type="button" class="absolute top-3 end-2.5 text-gray-400 hover:text-gray-900" data-modal-hide="category-delete-modal-{{ $category->id }}">
              &times;
          </button>
          <div class="p-4 text-center">
              ...
              <form method="POST" action="{{ route('category.destroy', $category->id) }}">
                ...
                @method('DELETE')
                  ...
                  <button type="button" data-modal-hide="category-delete-modal-{{ $category->id }}" class="px-5 py-2.5 ms-3 border rounded-lg">Batalkan</button>
              </form>
          </div>
      </div>
  </div>
