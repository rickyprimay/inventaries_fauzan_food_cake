<div id="user-delete-modal-{{ $user->id }}" tabindex="-1"
  ...
  <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="bg-white rounded-lg shadow-sm">
          <button type="button" class="absolute top-3 end-2.5 text-gray-400 hover:text-gray-900" data-modal-hide="user-delete-modal-{{ $user->id }}">
              &times;
          </button>
          <div class="p-4 text-center">
              ...
              <h3 class="mb-5 text-lg font-normal text-gray-500">Apakah anda yakin ingin menghapus user ini?</h3>
              <form method="POST" action="{{ route('user.destroy', $user->id) }}">
                  @csrf
                  @method('DELETE')
                  <button type="submit" class="text-white bg-red-600 hover:bg-red-800 px-5 py-2.5 rounded-lg">Ya, saya yakin</button>
                  <button type="button" data-modal-hide="user-delete-modal-{{ $user->id }}" class="px-5 py-2.5 ms-3 border rounded-lg">Batalkan</button>
              </form>
          </div>
      </div>
  </div>
