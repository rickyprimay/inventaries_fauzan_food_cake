<div id="stokistReport-delete-modal-{{ $stokistReport->id }}" tabindex="-1"
  ...
  <div class="relative p-4 w-full max-w-md max-h-full">
      <div class="bg-white rounded-lg shadow-sm">
          <div class="p-4 text-center">
              ...
              <form method="POST" action="{{ route('stokist-report.delete', $stokistReport->id) }}">
                @csrf
                @method('DELETE')
                  <button type="submit" class="text-white bg-red-600 hover:bg-red-800 px-5 py-2.5 rounded-lg">Ya, saya yakin</button>
                  <button type="button" data-modal-hide="stokistReport-delete-modal-{{ $stokistReport->id }}" class="px-5 py-2.5 ms-3 border rounded-lg">Batalkan</button>
              </form>
          </div>
      </div>
  </div>
